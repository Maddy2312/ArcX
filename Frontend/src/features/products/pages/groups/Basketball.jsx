...
const Basketball = () => {
  ...
  // Filter products for the Basketball category
  const basketballProducts = products.filter((p) => p.category === "Basketball");
    ...
    <div className="min-h-screen bg-white dark:bg-black text-black dark:text-white font-[Space_Grotesk,Inter,sans-serif]">
      ...
      <section className="relative py-32 bg-zinc-50 dark:bg-zinc-900">
        <div className="max-w-7xl mx-auto px-6">
          <span className="text-[10px] font-black tracking-[0.2em] uppercase text-black/40 dark:text-white/40">Performance Lab</span>
          <h1 className="text-6xl md:text-8xl font-black uppercase tracking-[-0.03em] leading-[0.9] mt-4">
            Basketball <br /> <span className="text-black/20 dark:text-white/20">Collection</span>
          </h1>
          <p className="max-w-md mt-8 text-black/60 dark:text-white/60 font-medium">
            Dominate the court with precision engineering. Discover our latest footwear and gear designed for elite court performance and explosive movement.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        <div className="flex items-center justify-between mb-12">
          <h2 className="text-2xl font-black uppercase tracking-tight">Active Inventory ({basketballProducts.length})</h2>
        </div>

        {basketballProducts.length === 0 ? (
          <div className="text-center py-24 border border-black/5 dark:border-white/5 rounded-3xl">
            <p className="text-black/40">No basketball gear found at the moment.</p>
          </div>
        ) : (
